...
class Wines(Base):
    def insert(self, userid, name, typed, pics, general_info,
               sight, nose, taste, notes):
        try:
            sql = ("INSERT INTO winesdb.wines"
                   " (wineid, userid, name, typed, pics, general_info,"
                   " sight, nose, taste, notes, created)"
                   " VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)")
            wineid = unicode(uuid.uuid4())
            created = datetime.datetime.utcnow()
            pics = json.dumps(pics)
            general_info = json.dumps(general_info)
            sight = json.dumps(sight)
            nose = json.dumps(nose)
            taste = json.dumps(taste)
